...
# Single-file H100 Nemotron benchmark for the customer-support SLA (v2):
...
RESULTS_ROOT="${RESULTS_ROOT:-${HOME}/results_h100_nemotron}"
...
REQUEST_RATES_3000=(1.157 3 5 10)
REQUEST_RATES_8000=(1.157 1.5 2 2.5)
OUTPUT_LENGTH=300
...
# Run the 4B speed baseline first, followed by the A3B quality candidate.
MODELS=(
  "$(model_ref NVIDIA-Nemotron-3-Nano-4B-FP8 nvidia/NVIDIA-Nemotron-3-Nano-4B-FP8)"
  "$(model_ref NVIDIA-Nemotron-3-Nano-30B-A3B-FP8 nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-FP8)"
)

# NVIDIA recommends FlashInfer's FP8 MoE path for Nemotron 3 Nano.
...
for model in "${MODELS[@]}"; do
  ...
  server_cmd=(
    ...
    --dtype auto
    --trust-remote-code
    # Nemotron-H contains FP8 Mamba projections for which vLLM has no
    # forced Triton linear kernel. Let vLLM select a compatible backend
    # for each layer on this H100 instead of failing during model startup.
    --linear-backend auto
  ...
  for input_len in "${INPUT_LENGTHS[@]}"; do
    if [[ "${input_len}" == "3000" ]]; then
      request_rates=("${REQUEST_RATES_3000[@]}")
    else
      request_rates=("${REQUEST_RATES_8000[@]}")
    fi
